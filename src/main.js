...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addToCart(productId, count) {
        ...
        if (productToAdd) {
            ...
            // Скидаємо лічильник до 1 після додавання
            quantity = 1;
    ...
    function update3DView(productIndex) {
        ...
        productBox.querySelector('.top').style.backgroundImage = `url(${sides[2]})`;
    ...
    addToCart3DBtn.addEventListener('click', () => {
        if (current3DProductIndex > -1) {
            addToCart(allProducts[current3DProductIndex].id, quantity);
        }
    });
    // --- КІНЕЦЬ ЗМІН ---
